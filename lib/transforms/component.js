...
function createComponent(type, props) {
  let id = "__cmp_" + ++elemGlobalId;
  return [
    t.identifier(id),
    t.callExpression(type, [processProps(props)].filter(Boolean))
  ];
}
...
function processChildren(parentId, children) {
  ...
  children.forEach(child => {
    ...
    if (t.isIdentifier(child.arguments[0])) {
      [id, newNode] = createComponent(child.arguments[0], child.arguments[1]);
    } else {
      [id, newNode] = createElement(child.arguments[0], child.arguments[1]);
    }
  ...
  path.traverse({
    ReturnStatement(path) {
      ...
      if (t.isIdentifier(node.arguments[0])) {
        [id, newNode] = createComponent(node.arguments[0], node.arguments[1]);
      } else {
        [id, newNode] = createElement(node.arguments[0], node.arguments[1]);
      }
